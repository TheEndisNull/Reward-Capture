...
model_df = Model(name="My data. Drift varies with reward",
                  drift=DriftReward(
                     driftRwd=Fittable(minval=.1, maxval=20),
                     driftPos=Fittable(minval=.1, maxval=20)),
...
# plt.show()
##
pyddm.plot.model_gui(model=model_df, conditions={"rwdType": [1,2], "colMatch": [1,2]}, sample=df_sample)
