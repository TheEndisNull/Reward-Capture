...
data <- read_csv(
  ...
  mutate(subj_idx = substr(subj_idx, 1, 3),
         rwdType = as.integer(str_replace_all(rwdType, c("none" = "0", "low" = "1", "high" = "2"))),
         tgtPos = as.integer(str_replace_all(tgtPos,  c("new" = "0", "1st" = "1", "2nd" = "2", "3rd" = "3"))),
         colPos = as.integer(str_replace_all(colPos,  c("none" = "0", "1st" = "1", "2nd" = "2", "3rd" = "3")))
         ) %>%
  filter(trialType == "RSVPtest",
         rt > quantile(data$rt, 0.025, na.rm = TRUE)
         ) %>%
  select(subj_idx, rt, response, rwdType, tgtPos, colPos) 
data <- mutate(data, colMatch = ifelse(tgtPos==colPos & rwdType!=0,2,
                                       ifelse(tgtPos!=colPos & rwdType!=0,1,0)),
               rt = rt/1000)
...
dataNonMatching <- filter(data, colMatch==1)
